...
namespace eae6320
{
	namespace Math
	{
		sVector sVector::WorldRight = sVector(1,0,0);
		sVector sVector::WorldUp = sVector(0, 1, 0);
		sVector sVector::WorldForward = sVector(0, 0, -1);
	}
}
...
float eae6320::Math::sVector::GetLength_Sqr() const
{
	return  (x * x) + (y * y) + (z * z);
}
