...
namespace eae6320
{
	...
	private:
		...
		{
			return "Boson HUANG's Triple A's Multi-player Zombie Survival First Person Shooter"
				" -- "
		...
		virtual void SwitchCamera(Camera* newCamera);
		// Initialization / Clean Up
		//--------------------------

		virtual cResult Initialize() override;
		virtual cResult CleanUp() override;
		virtual void InitializeActorWithGraphicAndTransformationInfo(Actor* o_actor, Graphics::cGeometry& thisGeo, Graphics::cEffect& thisEffect, Math::sVector i_initialLocation, Math::cQuaternion i_initialQuaternion);

		Camera* m_mainCamera;
		std::vector<Actor*> m_initial_ActorList;
		std::vector<Actor*> m_actorList;
		std::vector<std::pair<Graphics::cGeometry*, Graphics::cEffect*>> m_initial_RenderList;
		std::vector<std::pair<Graphics::cGeometry*, Graphics::cEffect*>> m_renderList;
		float m_backColor[4] = {0,0,0,1};
